...
{
	this->radius_ = radius;							//设置半径
	this->minPTs_ = minPTs;							//设置领域最小数据个数
	this->dimNum_ = DIME_NUM;						//设置数据维度
	...
	while (!ifs.eof())								//从文件中读取POI信息，将POI信息写入POI列表中
	{
		...
		dadaSets_.push_back(tempDP);				//将对象压入数据集合容器
		i++;										//计数+1
	}
	ifs.close();									//关闭文件流
	dataNum_ = i;									//设置数据对象集合大小为i
	for (unsigned long i = 0; i<dataNum_; i++)
	{
		SetArrivalPoints(dadaSets_[i]);				//计算数据点领域内对象
	}
	return true;									//返回
}

/***********************************************************
*函数名称		InitData(vector<double> data,double radius, int minPTs)
*参数			data:vector容器,radius设置半径,minPTs领域最小数据个数
*返回值			void
...
void DBSCAN::InitData(vector<double>& data, double radius, int minPTs,int N)
{
	...
	try
	{
		...
		for (int n = 0; n < len / N; n++)
		{
			...
			tempDP.SetDimension(tempDimData);			//将维度信息存入数据点对象内
			tempDP.SetDpId(n);							//将数据点对象ID设置为i
			tempDP.SetVisited(false);					//数据点对象isVisited设置为false
			tempDP.SetClusterId(-1);					//设置默认簇ID为-1
			this->dadaSets_.push_back(tempDP);				//将对象压入数据集合容器
		}
	}
	catch (exception* ex)
	{
	...
	this->dataNum_ = data.size() / N;							//设置数据对象集合大小为 data.size() / N
	for (unsigned long i = 0; i<dataNum_; i++)
	{
		SetArrivalPoints(this->dadaSets_[i]);				//计算数据点领域内对象
	}
}
...
bool DBSCAN::WriteToFile(char* fileName)
{
	...
	for (unsigned long i = 0; i<dataNum_; i++)                //对处理过的每个数据点写入文件
	{
		for (int d = 0; d<DIME_NUM; d++)                    //将维度信息写入文件
			of1 << dadaSets_[i].GetDimension()[d] << '\t';
		of1 << dadaSets_[i].GetClusterId() << endl;        //将所属簇ID写入文件
	}
	of1.close();    //关闭输出文件流
	return true;    //返回
}
...
bool DBSCAN::WriteToFile(fstream& out)
{
	try
	{
		for (unsigned long i = 0; i < dataNum_; i++)                //对处理过的每个数据点写入文件
		{
			...
			out << endl;
			out << "["<<dadaSets_[i].GetClusterId() <<"]"<< endl;        //将所属簇ID写入文件
		}
		out.close();    //关闭输出文件流
		return true;    //返回
	}
...
{
	for (unsigned long i = 0; i < dataNum_; i++)                //对每个数据点执行
	{
		double distance = GetDistance(dadaSets_[i], dp);    //获取与特定点之间的距离
		if (distance <= radius_ && i != dp.GetDpId())        //若距离小于半径，并且特定点的id与dp的id不同执行
			dp.GetArrivalPoints().push_back(i);            //将特定点id压力dp的领域列表中
	}
	if (dp.GetArrivalPoints().size() >= minPTs_)            //若dp领域内数据点数据量> minPTs执行
	{
...
bool DBSCAN::DoDBSCANRecursive()
{
	...
	for (unsigned long i = 0; i<dataNum_; i++)				//对每一个数据点执行
	{
		DataPoint& dp = dadaSets_[i];						//取到第i个数据点对象
...
{
	DataPoint& srcDp = dadaSets_[dpID];        //获取数据点对象
	...
	{
		DataPoint& desDp = dadaSets_[arrvalPoints[i]];    //获取领域内点数据点
